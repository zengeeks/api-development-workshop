...
Azure ポータル `https://portal.azure.com` を開き、画面上部の検索で「関数」と入力すると **関数アプリ** が表示されますので、クリックします。
...
Visual Studio から前述で作った Function App へデプロイを行います。手順は以下の2つです。
...
発行が確認出来たら、次は Azure ポータルから確認してみます。
